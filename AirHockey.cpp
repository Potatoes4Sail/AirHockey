...
void launchDirectXWindow() {// Launches new DirectX process.
    system("2D_graphics\\DirectX_window\\launch_DirectX.bat");
    // ShellExecuteA(NULL, NULL, "2D_graphics/DirectX_window/launch_DirectX.bat", NULL, NULL, SW_SHOW); // bat file kills any existing DirectX processes, then launches a new one.
    this_thread::sleep_for(chrono::seconds{ 1 }); // Delay needed to allow DirectX window to launch fully. 2 seconds works consistently
}
